/* Copyright 2023 (c) Michael Bell
   ...
   the MSB of the word.
   */
module spi_ram_controller #(parameter DATA_WIDTH_BYTES=1, parameter ADDR_BITS=16) (
    input clk,
    input rstn,
...
`define max(a, b) (a > b) ? a : b
    ...
    localparam SPI_ADDR_BITS = 16;  // Always send 16-bit addresses on SPI bus
    ...
    reg writing;
    reg spi_miso_buf;
    reg [SPI_ADDR_BITS-1:0]   addr;  // Always 16-bit for SPI protocol
    reg [DATA_WIDTH_BITS-1:0] data;
    reg [$clog2(`max(DATA_WIDTH_BITS,SPI_ADDR_BITS))-1:0] bits_remaining;
        ...
        if (!rstn) begin
            ...
            if (fsm_state == FSM_IDLE) begin
                ...
                if (bits_remaining == 0) begin
                    ...
                    if (fsm_state == FSM_CMD)       bits_remaining <= SPI_ADDR_BITS-1;
    ...
    always @(posedge clk) begin
        if (fsm_state == FSM_IDLE && (start_read || start_write)) begin
            addr <= {{(SPI_ADDR_BITS-ADDR_BITS){1'b0}}, addr_in};  // Zero-extend to 16 bits
            writing <= start_write;
        end else if (fsm_state == FSM_ADDR) begin
            addr <= {addr[SPI_ADDR_BITS-2:0], 1'b0};
        end
    end
    ...
    assign spi_mosi = fsm_state == FSM_IDLE ? 1'b0 :
                      ...
                      fsm_state == FSM_ADDR ? addr[SPI_ADDR_BITS-1] :
