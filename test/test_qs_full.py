...
"""
Full quadratic sieve test - Factor N = 604227100699 = 677779 × 891481

This is the complete factorization test using the full 64KB sieve array
and a large factor base (primes up to 547). Expected runtime: ~30 seconds.
"""
...
# Test parameters
N = 604227100699  # = 677779 × 891481
FACTOR_BASE_BOUND = 1103  # Large factor base
SIEVE_SIZE = 65536  # Full 64KB sieve array
THRESHOLD = 20  # Lowered threshold to get more candidates
...
@cocotb.test()
async def test_factor_604227100699(dut):
    """Factor N = 604227100699 = 677779 × 891481 using hardware quadratic sieve."""
    dut._log.info("=" * 80)
    dut._log.info("QUADRATIC SIEVE FULL TEST - FACTOR N = 604227100699")
    dut._log.info("=" * 80)
    ...
    dut._log.info(f"   Factor base (last 10): {qs.factor_base[-10:]}")
    ...
    while init_cycles < max_init_cycles:
        ...
        # Log progress every 100k cycles
        if init_cycles - last_log >= 100000:
            dut._log.info(
                f"   Initialization progress: {init_cycles / 1000000:.1f}M cycles..."
            )
    ...
    dut._log.info(
        f"   ✓ SPI RAM initialized ({init_cycles} cycles = {init_cycles / 10000:.1f}ms @ 100MHz)"
    )
    ...
    for prime_idx, prime in enumerate(qs.factor_base):
        if prime_idx % 10 == 0 or prime_idx < 3:
            dut._log.info(
        ...
        # Report progress every 10 primes
        if prime_idx % 10 == 0 and prime_idx > 0:
            dut._log.info(
                f"   Progress: {prime_idx}/{len(qs.factor_base)} primes processed, {len(all_reports)} unique candidates so far"
            )
    ...
    for idx, addr in enumerate(sorted(all_reports)):
        ...
        if factors is not None:
            ...
            if factors_tuple not in seen_factors:
                ...
                if smooth_count <= 10:
                    dut._log.info(
                        f"   ✓ addr=0x{addr:04x}: smooth! (factors: {len(factors)} primes)"
                    )
        else:
            not_smooth_count += 1
    ...
    if factors_found is not None:
        ...
        assert (p == 677779 and q == 891481) or (
            p == 891481 and q == 677779
        ), f"Expected factors 677779 and 891481, got {p} and {q}"
        dut._log.info("✓✓✓ FACTORIZATION VERIFIED ✓✓✓")
        dut._log.info("✓✓✓ SUCCESSFULLY FACTORED 604227100699 = 677779 × 891481 ✓✓✓")
    else:
